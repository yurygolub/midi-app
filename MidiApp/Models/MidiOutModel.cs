...
public class MidiOutModel
{
    ...
    public async Task StartPlayingAsync(int deviceIndex, string filePath, CancellationToken token)
    {
        ...
        this.State.Output += $"Format {midiFile.FileFormat}, Tracks {midiFile.Tracks}, Delta Ticks Per Quarter Note {midiFile.DeltaTicksPerQuarterNote}{Environment.NewLine}";
        ...
        for (int i = 0; i < midiFile.Tracks; i++)
        {
            foreach (MidiEvent midiEvent in midiFile.Events[i])
            {
                ...
                {
                    this.State.Output += $"{ToMeasuresBeatsTicks(midiEvent.AbsoluteTime, midiFile.DeltaTicksPerQuarterNote, timeSignature)} {midiEvent}{Environment.NewLine}";
                }
